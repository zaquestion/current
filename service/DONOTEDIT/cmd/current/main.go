...
import (
	...
	// This Service
	"github.com/zaquestion/current/service/DONOTEDIT"
...
func main() {
	...
	*/
	...
	{
		//postlocationfrombigbrotherDuration := duration.With(metrics.Field{Key: "method", Value: "PostLocationFromBigBrother})"
		//postlocationfrombigbrotherLogger := log.NewContext(logger).With("method", "PostLocationFromBigBrother)")

		postlocationfrombigbrotherEndpoint = addsvc.MakePostLocationFromBigBrotherEndpoint(service)
		//postlocationfrombigbrotherEndpoint = opentracing.TraceServer(tracer, "PostLocationFromBigBrother)")(postlocationfrombigbrotherEndpoint)
		//postlocationfrombigbrotherEndpoint = addsvc.EndpointInstrumentingMiddleware(postlocationfrombigbrotherDuration)(postlocationfrombigbrotherEndpoint)
		//postlocationfrombigbrotherEndpoint = addsvc.EndpointLoggingMiddleware(postlocationfrombigbrotherLogger)(postlocationfrombigbrotherEndpoint)
	}

	var getlocationEndpoint endpoint.Endpoint
	{
		//getlocationDuration := duration.With(metrics.Field{Key: "method", Value: "GetLocation})"
		//getlocationLogger := log.NewContext(logger).With("method", "GetLocation)")

		getlocationEndpoint = addsvc.MakeGetLocationEndpoint(service)
		//getlocationEndpoint = opentracing.TraceServer(tracer, "GetLocation)")(getlocationEndpoint)
		//getlocationEndpoint = addsvc.EndpointInstrumentingMiddleware(getlocationDuration)(getlocationEndpoint)
		//getlocationEndpoint = addsvc.EndpointLoggingMiddleware(getlocationLogger)(getlocationEndpoint)
	}
	...
	go func() {
		...
		s := grpc.NewServer()
		pb.RegisterCurrentServiceServer(s, srv)
