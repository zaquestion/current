...
import (
	...
	"github.com/zaquestion/current/current-service/handlers"
	"github.com/zaquestion/current/current-service/middlewares"
...
func Run(cfg Config, logger log.Logger) {
	...
	// Endpoint domain.
	var (
		postlocationbigbrotherEndpoint = svc.MakePostLocationBigBrotherEndpoint(service)
		postlocationtaskerEndpoint     = svc.MakePostLocationTaskerEndpoint(service)
	...
	go func() {
		...
		s := grpc.NewServer()
		pb.RegisterCurrentServer(s, srv)
